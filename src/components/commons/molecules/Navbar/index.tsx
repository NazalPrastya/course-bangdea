...
const Navbar = () => {
  ...
  return (
    <>
      <header className="absolute top-4 z-50 mt-2 h-12 flex items-center justify-between bg-transparent px-12 w-(--my-width-nav)">
        ...
        >
          <div className="flex h-full flex-col items-center justify-center">
            ...
            {listNavbar.map((item, index) => (
              <p
                key={index}
                className="w-full cursor-pointer px-4 py-2 text-lg font-semibold text-(--text-color-black) opacity-80 transition duration-300 ease-in-out hover:bg-(--bg-primar-cray) hover:opacity-100"
